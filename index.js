...
/*
var nyFarge = document
  .getElementById("ny_farge")
  .addEventListener("click", changeColor, rgbTilTall);
...
function rgbTilTall() {
  ...
  var convertColor = "rgb(" + red + "," + green + "," + blue + ")";
...
function hentEnBox() {
    for (let i = 0; i < 6; i++) {
        var randomBox = Math.floor(Math.random() * colorBox.length);
        randomBox[i] = test
        console.log(randomBox);

  }
}
var boks = document.querySelectorAll('div[name="colorbox"]')
var randomIndex = Math.floor(Math.random() * boks.length);
var randomDivBox = boks[randomIndex];
console.log(randomDivBox);
*/
...
const CSS_COLOR_NAMES = {
 ...
 function rgbTilTall() {
   ...
   document.getElementById("red").innerText = rgbValues[1];
   document.getElementById("green").innerText = rgbValues[2];
   document.getElementById("blue").innerText = rgbValues[3];
   ...
   for (let i = 0; i < colorBox.length; i++) {
     colorBox[i].onclick = function() {
       if (this.dataset.color === correctColor) {
         ...
         coretktAnswer.style.backgroundColor = "black";
